...
file_to_plot = full_path.split('/')[4]
folder_path = full_path.split('/')[0]+"/"+full_path.split('/')[1]+"/"+full_path.split('/')[2]+"/"+full_path.split('/')[3]+"/"

#file_path = "Templates/PostCodes/"

#Get file to plot from from sys
#file_to_plot = sys.args[1]
#file_to_plot = "Casey - Pulsz - Insert - 1.svg"
...
if 'Pulsz' in file_to_plot:
    sheet_prefix = 'Pulsz'
elif 'GP' in file_to_plot:
    sheet_prefix = 'GP'
elif 'Chumba' in file_to_plot:
    sheet_prefix = 'Chumba'
elif 'LL' in file_to_plot:
    sheet_prefix = 'LL'

plot_type = "Insert"

if 'Envelope' in file_to_plot:
        plot_type = "Envelope"

worksheet_name = file_to_plot.split('-')[0]
...
tracker_worksheet = sheet.worksheet(worksheet_name)
...
p_df = p_df[p_df[querying_for] != 'Used']

#p_df = p_df.query("@querying_for != True")

code = p_df["Base64 Image"].iloc[0]
print("Postal Code is: "+ str(code))

postal_code = code

column = tracker_worksheet.find(querying_for)
row = tracker_worksheet.find(str(code))
column = string.ascii_uppercase[column.col - 1] 
row = row.row
cell = column+str(row)
tracker_worksheet.update(cell, "Used")
