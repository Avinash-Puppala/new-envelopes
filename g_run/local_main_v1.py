...
creds = ServiceAccountCredentials.from_json_keyfile_name('pushin-the-envelope-4eb6601cb3bc.json',scope)
...
def failed_plot(file):
    if "Envelope" in file:
        ...
        etry_list = r.rpush('retry_inserts', file)
    ...
    todays_value = pulled_row[str(fd)].iloc[0]
    today_col = tracker_worksheet.find(str(fd))
...
def successful_plot(file):
    ...
    new_week_val = current_week_value + 1
    new_val = todays_value + 1
